...
/*   Updated: 2018/08/18 19:43:07 by pidecler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		check_border(char c, t_size size, int i, int j)
{
	if (i > 0 && j > 0 && i < size.x - 1 && j < size.y - 1 && c != ' ')
		return (1);
	else if (i > 0 && i < size.x - 1 && (j == 0 || j == size.y - 1) && c != 'B')
		return (1);
	else if (j > 0 && j < size.y - 1 && (i == 0 || i == size.x - 1) && c != 'B')
		return (1);
	return (0);
}
...
static int		check_lil(t_size size, char **str)
{
	...
	i = 0;
	if ((j = 1) && size.x == 1 && size.y == 1 && str[0][0] == 'A')
		return (1);
	else if (str[0][0] != 'A')
		return (0);
	...
	else if (size.x > 1 && size.y == 1 && str[0][0] == 'A')
	{
		while (i < size.x - 1)
			if (str[0][i++] != 'B')
				return (0);
...
t_size			*check_4(char **str)
{
	...
	if ((size->y == 1 || size->x == 1) && check_lil(*size, str))
		return (size);
	if ((j = -1) && (str[0][0] != 'A' || str[0][size->x - 1] != 'C'
		|| str[size->y - 1][0] != 'C' || str[size->y - 1][size->x - 1] != 'A'))
		return (NULL);
	while (++j < size->y)
	{
		i = -1;
